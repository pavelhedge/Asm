...
выгрузили обратно */
	...
	sec
	rol temp1				; просто сдвигаем единицу по регистру
	
.if number_of_digits==8			; Если число разрядов - восемь, проверяем
	brlo I_disp_a	 		; единицу по флагу С в SREG 
	ldi temp1, 0xFE 		; если убежала - ставим на место
 I_disp_a:
 
.else	sbrs temp1, number_of_digits	; проверяя, чтоб не убежал слишком далеко
	ldi temp1, 0xFE 		; если убежал - ставим на место
.endif
