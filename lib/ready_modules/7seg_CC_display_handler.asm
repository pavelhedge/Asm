...
.if digit_number=1			; под значения, выводимые на дисплей
digit1: .byte 1
.elif digit_number=2
digit1: .byte 1
digit2: .byte 1
...
andi temp2, low(0xFF<<(digit_number))	; в индикации от изменений под маской
	sec					; флаг нужен, чтоб при rol в 0 бит сдвигалась 1
	rol temp1				; просто сдвигаем ноль по регистру
	
.if digit_number==8			; Если число разрядов - восемь, проверяем
	brlo I_disp_a	 		; ноль по флагу С в SREG 
	ldi temp1, 0b11111110 		; если убежал - ставим на место
 I_disp_a:
 
.else	sbrs temp1, digit_number	; проверяя, чтоб не убежал слишком далеко
	ldi temp1, 0b11111110 		; если убежал - ставим на место
.endif

andi temp1, 0xFF>>(8-digit_number)	; А здесь неиспользуемые разряды
 ...
 */
	...
	cpi XL, (low(digit1)+digit_number)	; адрес следующей цифры сам 
